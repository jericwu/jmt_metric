	...
	private void generateABU(PopulationVector N, int s) throws InternalErrorException {
		...
		for (int i = 0; i < matrixSize; i++) {
			for (int j = 0; j < matrixSize; j++) {
				A[i][j] = BigRational.ZERO; // We do not use writeToA() as this would remove j from the uncomputable set and we are in the initialisation phase.
		...
		for (int i = 0; i <= limit; i++) { // (* Add to the linear system all convolution equations *)
			...
			if (dM.sum() == R - 1) {
				...
				for (int k = 1; k <= M; k++) {
					...
					writeToA(row, pos, BigRational.ONE);

					writeToA(row, i, new BigRational(-1));
					for (int r = 1; r <= s - 1; r++) {
						int pos2 = sz * r + canonicalMVCalc.vectoint(dMPlusk);
						writeToA(row, pos2, qnm.getDemandAsBigRational(k - 1, r - 1).negate());
					}
		...
		for (int i = 0; i <= limit; i++) { // (* Add to the linear system all population constraints *)
			...
			if (dM.sum() == R - 1) {
				for (int r = 1; r <= s - 1; r++) {
					row++;
					writeToA(row, i, N.getAsBigRational(r - 1));
					int pos = sz * r + i;
					writeToA(row, pos, qnm.getDelayAsBigRational(r-1).negate());
					MultiplicitiesVector dMPlusk = dM.copy();
					for (int k = 1; k <= M; k++) {
						dMPlusk.plusOne(k);
						...
						writeToA(row, pos, bf.negate().multiply(qnm.getDemandAsBigRational(k - 1, r - 1)));
						dMPlusk.restore();
					}
				}
			}
			for (int r = 0; r <= s - 1; r++) {
				row++;
				int pos = sz * r + i;
				writeToA(row, pos, N.getAsBigRational(s - 1));
				UList.add(new Tuple<Integer, Integer>(row, pos));
				B[row][pos] = qnm.getDelayAsBigRational(s-1);
				MultiplicitiesVector dMPlusk = dM.copy();
				for (int k = 1; k <= M; k++) {
					dMPlusk.plusOne(k);
					int f = dM.get(k - 1) + qnm.multiplicities.get(k - 1);
