...
/**
 * <p><b>Name:</b> DispatcherJMVAschema</p> 
 ...
 * @version 2.0
 */
public class DispatcherJMVAschema {
	 ...
	 * model to be solved
	 */
	public DispatcherJMVAschema(String absolutePath) {
	 ...
	 * model to be solved
	 */
	public DispatcherJMVAschema(File model) {
	...
	public boolean solveModel() {
		...
		//save the file in the same directory of model file, with the same name preceded by "sim_"
		...
		try {
			...
			if (!automaticSeed) {
				//if !automaticSeed, then a user defined seed is present
		...
		//the file has been saved in the same directory of model file, with the same name preceded by "res_sim_"
	...
	public boolean solveModelNewTransform() {
		boolean transfSuccess = false;
		//get file name: same directory of model file, with the same name preceded by "sim_"
		String modelDefinitionParent = modelDefinition.getParent();
		...
		if (!transfSuccess) {
			...
			try {
				...
				logger.error("Cannot parse correctly output JMVA model: " + e1.getMessage());
				return false;
			} catch (IOException e2) {
				logger.error("I/O error opening output JMVA model: " + e2.getMessage());
				return false;
			}
	...
	public double checkSimProgress() {
		...
		try {
			...
			if (pauseSim()) {
				double progress = net.getNetSystem().checkProgress(net);
				restartSim();
				return progress;
			} else {
				return 0.0;
	...
	private boolean restartSim() {
		return net.getNetSystem().restartFromPause();
	}
