...
public class GraphPanel extends WizardPanel implements ExactConstants {
	...
	static final Dimension DIM_SPINNER = new Dimension(60, 20);
	...
	private void initGraphics() {
		...
		index = new JComboBox<String>(ExactConstants.INDICES_TYPES);
		...
		if (model.isWhatifAlgorithms()) {
			Arrays.fill(algorithms, model.getWhatifAlgorithms().iterator().next());
	...
	private void updateIndex() {
		...
		if (!current.equals(currentIndex)) {
			...
			if (currentIndex.equals(ExactConstants.INDICES_TYPES[4])) {
				// If System Power is selected, removes the station column
			...
			if (AGGREGATE_TYPES_SET.contains(currentIndex)) {
				...
				for (SolverAlgorithm algo : executedAlgorithms) {
					...
					// System response time
					if (currentIndex.equals(AGGREGATE_TYPES[1])) {
						graph.draw(i++, model.getGlobalR(algo, false));
					}
					// System throughput
					...
					// Number of customers
					else if (currentIndex.equals(AGGREGATE_TYPES[3])) {
	...
	private void paintIndexAtRow(int rowNum) {
		...
		// There is no station column for System Power
	...
	protected class LinesTable extends JTable {
		...
		public LinesTable() {
			...
			JComboBox<String> classCombo = new JComboBox<String>();
			// Null elements
			classCombo.addItem("");
			// Aggregate measures
			...
			JComboBox<String> stationsCombo = new JComboBox<String>();
			JComboBox<String> uStationsCombo = new JComboBox<String>();
			...
			JComboBox<SolverAlgorithm> algorithmCombo = new JComboBox<SolverAlgorithm>();
