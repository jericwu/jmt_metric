...
import jmt.jmva.gui.exact.panels.SystemPowerPanel;
...
public class ExactWizard extends Wizard {
	...
	// New Bertoli Marco
	private ModelLoader modelLoader = new ModelLoader(ModelLoader.JMVA, ModelLoader.JMVA_SAVE);
	// End
	...
	protected void switchToSimulator() {
		...
		JSIMWizMain jsim = new JSIMWizMain(output);
	...
	private void solve() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, e.getMessage(), "Unsupported model", JOptionPane.WARNING_MESSAGE);
			return;
		} catch (InputDataException e) {
			JOptionPane.showMessageDialog(this, e.getMessage(), "Input data error", JOptionPane.ERROR_MESSAGE);
			return;
	...
	private void createSolutionWindow() {
		...
		if (selector != null && data.isClosed() && data.isWhatifAlgorithms()) {
			for (SolverAlgorithm algorithm : data.getWhatifAlgorithms()) {
				...
				algPanel.addSolutionPanel(new SystemPowerPanel(this, algorithm));
			...
			selector.addSolutionPanel(new SystemPowerPanel(this, algorithm));
			jtp.add(selector);
		} else {
			SolverAlgorithm algorithm = data.getAlgorithmType();
			jtp.add(new ThroughputPanel(this, algorithm));
			jtp.add(new QueueLenPanel(this, algorithm));
			jtp.add(new ResTimePanel(this, algorithm));
			jtp.add(new SystemRespTimePanel(this,algorithm));
			jtp.add(new UtilizationPanel(this, algorithm));
			jtp.add(new SystemPowerPanel(this, algorithm));
		}
		/* END */
