 ...
 */
public class DispatcherJSIMschema {
	 ...
	 * model to be solved
	 */
	public DispatcherJSIMschema(String absolutePath) {
	 ...
	 * model to be solved
	 */
	public DispatcherJSIMschema(File model) {
	...
	public boolean solveModel() throws Exception {
		...
		if (automaticSeed) {
			...
			simulationSeed = sim.getEngine().nextInt();
			if (simulationSeed < 0) {
	...
	public synchronized void refreshTempMeasures() {
		...
		if (pauseSim()) {
			...
			try {
				progress = net.getNetSystem().checkProgress(net);
