 ...
 */
public class RefSourcesPanel extends WizardPanel implements CommonConstants {
	...
	public RefSourcesPanel(ClassDefinition classes, StationDefinition stations, SimulationDefinition sim) {
		...
		public TableCellEditor getCellEditor(int row, int column) {
			if (column == 1) {
				if (classData.getClassType(classData.getClassKeys().get(row)) == CLASS_TYPE_OPEN) {
					return combos.getEditor(stationData.getStationKeysRefStation());
				} else {
