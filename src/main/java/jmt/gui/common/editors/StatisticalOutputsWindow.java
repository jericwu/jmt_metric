...
public class StatisticalOutputsWindow extends JMTFrame {
	...
	public static final int TYPE_DENSITY_LINE = 2;
	...
	public StatisticalOutputsWindow(MeasureDefinition measureDefinition, int measureIndex) {
		...
		String measureType = measureDefinition.getMeasureType(measureIndex);
		inverseMeasure = EngineUtils.isInverseMeasure(EngineUtils.decodeMeasureType(measureType));
		try {
	...
	public void initGUI() {
		...
		reDrawButton.setToolTipText("Redraw the graph with the new minimum, maximum values, number of intervals" +
				" and graphType on the basis of selected samples.");
		...
		if (inverseMeasure) {
			JLabel warningLabel = new JLabel("<html>Please note that statistics for Throughput, Drop Rate and System Power indices are based on inter-departure times.</html>");
	...
	private void saveChartData(File targetFile, DistributionType type) throws IOException {
		CSVLogger logger = new CSVLogger(targetFile, new String[] {CSV_COLUMN_INDEX, CSV_COLUMN_START, CSV_COLUMN_END, CSV_COLUMN_VALUE}, false, 
				Defaults.get("loggerDelimiter"), Defaults.get("loggerDecimalSeparator"));
		try {
